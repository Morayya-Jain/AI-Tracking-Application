...
# Runtime hook for embedded API keys
# This file is generated by build_macos.sh with actual key values
RUNTIME_HOOK = str(SPEC_DIR / 'runtime_hook.py')
runtime_hooks = [RUNTIME_HOOK] if os.path.exists(RUNTIME_HOOK) else []

if runtime_hooks:
    print(f"Using runtime hook: {RUNTIME_HOOK}")
else:
    print(f"Warning: Runtime hook not found at {RUNTIME_HOOK}")
    print("API keys may not be embedded in the build.")
